...
let JSONCrush = {
  crush: function crush(string) {
    ...
    if (crushed.b.length) crushedString += delimiter + crushed.b; // fix issues with some links not being recognized properly

    crushedString += '_'; // return crushed string

    return crushedString;
  },
  uncrush: function uncrush(string) {
    // remove last character
    string = string.substring(0, string.length - 1); // unsplit the string using the delimiter
...
var JSONCrushSwap = function JSONCrushSwap(string) {
  ...
  var swapInternal = function swapInternal(string, g) {
